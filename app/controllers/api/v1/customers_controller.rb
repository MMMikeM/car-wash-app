class Api::V1::CustomersController < Api::V1::ApiController
  ...
  def password
    @password ||= params[:password].present? ? params[:password] : [params[:name].to_s.tr(" ", "").first(6), params[:contact_number]].join("_")
  end

  def permitted_params
    params.require(:customer)
          .permit(:name, :email, :contact_number, :opted_for_marketing, :total_points, :loyalty_enabled, vehicles_attributes: [:registration_number])
          .merge({
            :password => password,
